...
if (AI_SELECTED === 'GEMINI' && !process.env.GEMINI_KEY) {
  throw new Error('Você precisa colocar uma key do Gemini no .env! Crie uma gratuitamente em https://aistudio.google.com/app/apikey?hl=pt-br');
}

if (AI_SELECTED === 'GPT' && (!process.env.OPENAI_KEY || !process.env.OPENAI_ASSISTANT)) {
  throw new Error('Para utilizar o GPT você precisa colocar no .env a sua key da openai e o id do seu assistente.');
}
